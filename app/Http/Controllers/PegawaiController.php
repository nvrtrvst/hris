...
class PegawaiController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            // Import now throws pre-grouped error message in 'import' key
            $message = $errors['import'] ?? null;
            if (! $message) {
                // Fallback: group manually for edge cases
                $grouped = collect($errors)
                    ->map(fn ($msgs, $key) => [
                        'row' => (int) explode('.', $key)[0] + 2,
                        'msg' => is_array($msgs) ? reset($msgs) : $msgs,
                    ])
                    ->groupBy('msg')
                    ->map(fn ($items, $msg) => '• '.$msg.' (baris '.collect($items)->pluck('row')->unique()->implode(', ').')')
                    ->values()
                    ->implode("\n");
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Auto-assign atasan jika belum di-set manual
        $pegawai->load('units');
        $pegawai->autoAssignAtasan();

        // Sync User.unit_sekolah_id ke unit primari pegawai
        if ($pegawai->user_id && ! empty($syncUnits)) {
            $primaryUnitId = null;
            foreach ($syncUnits as $unitId => $attrs) {
            ...
            if (! $primaryUnitId) {
                $primaryUnitId = array_key_first($syncUnits);
            }
            User::where('id', $pegawai->user_id)->update(['unit_sekolah_id' => $primaryUnitId]);
        }
