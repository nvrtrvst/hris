...
const buildMapelGroups = (children) => {
    const groups = [];
    const others = [];
    for (const c of children) {
        if (!c.jadwal || c.is_tugas_luar) {
            others.push(c);
            continue;
        }
        const nama = c.jadwal.mata_pelajaran?.nama || 'Lainnya';
        const kelas = c.jadwal.kelas_label || '';
        const key = `${nama}|||${kelas}`;
        let g = groups.find((x) => x.key === key);
        if (!g) {
            g = { key, nama, kelas, items: [] };
            groups.push(g);
        }
        g.items.push(c);
    }
    groups.sort((a, b) => (a.items[0].jadwal?.jam_mulai || '').localeCompare(b.items[0].jadwal?.jam_mulai || ''));
    for (const g of groups) g.items.sort((a, b) => (a.jadwal?.jam_mulai || '').localeCompare(b.jadwal?.jam_mulai || ''));
        ...
        <div className="border-b border-border bg-surface/60 px-4 py-3 sm:px-6">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2 text-xs font-semibold text-text-secondary">
                    ...
                    <span className="rounded-full bg-border px-2 py-0.5 text-[11px] font-bold text-text-primary">{total} orang</span>
                </div>
...
export default function Index({ auth, presensis, pegawai, filters = {}, units, stats }) {
    ...
    const statsCards = [
        ...
        { key: 'telat', label: 'Terlambat', value: s.telat, Icon: AlarmClock, iconBg: 'bg-amber-100', iconCls: 'text-amber-600', filter: { status_filter: 'telat', lembur_filter: '', lokasi_filter: '', suspicious_filter: '' }, active: statusFilter === 'telat' },
