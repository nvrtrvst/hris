...
- **Slide sesi mengajar**: "Sesi" = rantai JP konsekutif **forward-only** dari anchor, gap ≤ 60 menit (`GAP_SESI_MENIT`), kriteria sama (pegawai+hari+unit+kelas+mapel). `MobileController::sesiMengajar()` walk forward saja (tidak backward). Window check: `[anchor.jam_mulai, lastJP.jam_selesai + grace]`. Cover forward-only: `coverSesiMengajar` loop sisa sesi, skip anchor. JP sebelum anchor TIDAK di-cover (alpa jika tidak di-slide sendiri). `presensiKeluarTarget` cari last JP dalam sesi forward. Cover rows isi `keterangan` provenance ("auto-cover dari slide JP #X pukul HH:MM"). FE `groupConsecutiveJadwals` pakai aturan sama (gap ≤ 60 menit).
